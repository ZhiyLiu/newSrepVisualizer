...
class qSlicervisualizerModuleGenericTest(unittest.TestCase):

  def setUp(self):
    import slicer
    self.assertTrue(hasattr(slicer, 'modules'))
    self.assertTrue(hasattr(slicer.modules, 'visualizer'))
    self.module = slicer.modules.visualizer

  def test_file_attribute(self):
    self.assertRegexpMatches(__file__, r'qSlicervisualizerModuleGenericTest.py[c]?$')

  def test_name_attribute(self):
    self.assertEqual(__name__, 'qSlicervisualizerModuleGenericTest')

  def test_module_valid(self):
    self.assertNotEqual(self.module, None)
    self.assertEqual(self.module.name, 'visualizer')
